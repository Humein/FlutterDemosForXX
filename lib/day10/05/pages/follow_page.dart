...
class _FollowerPageState extends State<FollowerPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: Text("关注者"),),
        body: Center(
            child: _buildByState(_state)) //根据状态控制显示
    );
  }
  ...
  _fetchData() async {
    try {
      _followers = await GithubApi.getUserFollowers(login: "toly1994328"); //调用api
    } catch (e) {
      print(e);
  ...
  StateType _checkState() {
    ...
    if(_followers==null){//当前数据对象为空
      return StateType.loading;
    }else if(_followers.isEmpty){//当前列表无数据
