...
class BiggerConfig {
  ...
  const BiggerConfig({this.rate = 3,
    this.isClip = true,
    this.radius = 30,
...
class BiggerView extends StatefulWidget {
  BiggerView({Key key,
    this.config = const BiggerConfig(),
    @required this.image})
      : super(key: key);
  final BiggerConfig config;
  final ImageProvider image; //图片提供器
  @override
  _BiggerViewState createState() =>
      _BiggerViewState();
}
...
class _BiggerViewState extends State<BiggerView> {
  ...
  Widget build(BuildContext context) => GestureDetector(
          onPanDown: (detail) { //点击回调，更新点位，可以画大图
            posX = detail.localPosition.dx;
            posY = detail.localPosition.dy;
            setState(() => canDraw = true);
          },
          onPanUpdate: (detail) { //手指移动，更新点位，并判断区域是否在图中，不再则不更新
            posX = detail.localPosition.dx;
            posY = detail.localPosition.dy;
          ...
          child: _image!=null?_buildImage(_image): Center(child: CircularProgressIndicator(),) ,
  ...
  Widget _buildImage(ui.Image image) {
    ...
    return Container(
      width: width,
      height: height,
      child: CustomPaint( //使用CustomPaint承载BiggerPainter
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_img != null) {
      Rect src = Rect.fromLTRB(
          0, 0, _img.width.toDouble(), _img.height.toDouble());
      canvas.drawImageRect(_img, src, rect, _paint); //绘制原图像
      if (_canDraw) {
        var offSetY = _y > 3 * _radius ? -2 * _radius : 2 *
            _radius; //当圆形区域出顶部进行校正
        circlePath.addOval(
            Rect.fromCircle(center: Offset(_x, _y + offSetY), radius: _radius));
        if (_config.isClip) { //根据是否需要圆形裁剪进行绘制
          canvas.clipPath(circlePath);
          canvas.drawImage(_img, Offset(
              -_x * (_config.rate - 1), -_y * (_config.rate - 1) + offSetY),
              _paint);
          canvas.drawPath(circlePath, _paint);
        } else {
          canvas.drawImage(
              _img, Offset(-_x * (_config.rate - 1), -_y * (_config.rate - 1)),
              _paint);
        }
      }
    }
