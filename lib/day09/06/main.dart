...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home:Scaffold(
        body: Center(child: GithubShower(),),
      ),
    );
  }
}
...
class _GithubShowerState extends State<GithubShower> {
  ...
  Widget build(BuildContext context) {
    //加载中视图
    var loadingView= Container(height: 80,child: Center(child: CircularProgressIndicator(),));
    //完成后视图
    var panel= GithubUserPanel(user: _user,color: Colors.black,);
    return Card(child:  _user != null ? panel:loadingView );//根据状态控制显示
  }
  _fetchData() async {//获取数据
    _user = await GithubApi.getUser(login: "mikyou");//调用api
    setState(() {});//渲染布局
  }
}