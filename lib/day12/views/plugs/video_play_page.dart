...
class _VideoPlayerViewState extends State<VideoPlayerView> {
  ...
  void initState() {
    super.initState();
    playUrls.add("/data/data/com.toly1994.flutter_journey/cache/sh.mp4"); //初始化数据
  ...
  Widget build(BuildContext context) {
    ...
    var btns = Row(children: <Widget>[//三个按钮
      InkWell(onTap: prev, //前一曲
          child: Icon(Icons.keyboard_arrow_left,
              size: 40, color: Colors.deepOrangeAccent)),
      InkWell(onTap: () {_playing ? pause() : play();}, //播放和暂停切换
    ...
    var progress = Container(height: 2,//进度条
        child: LinearProgressIndicator(value: _progress,
          valueColor: AlwaysStoppedAnimation<Color>(Colors.deepOrangeAccent)));
    var panel = Column(
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[video, progress,
        Row(children: <Widget>[btns, Expanded(child: seekBar,)],)],
    );
    return MaterialApp(
      title: 'Video Demo',
      home: Scaffold(
  ...
  Future<void> initPlayer() async {
    ...
    print("-----初始化完成-----size:$size----position:$position---duration:$duration--");
    _controller.addListener(() async {//对播放进行监听
      if (_controller.value.isPlaying) {//当播放时更新进度
        var position = _controller.value.position;
        if (position.inSeconds == _duration) {//说明播放结束
          next();
        }
